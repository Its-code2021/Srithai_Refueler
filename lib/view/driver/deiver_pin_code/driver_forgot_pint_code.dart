...
import 'driver_forgot_confirm_pint_code.dart';

void main() => runApp(Driver_Forgot_Pin_Code());

// ignore: use_key_in_widget_constructors
class Driver_Forgot_Pin_Code extends StatelessWidget {
                ...
                child: Padding(
                    ...
                    child: PinCodeTextField(
                      ...
                      onCompleted: (v) {
                        ...
                        Driver_currentText_Chang_Pin = currentText;
                        var List_number;
                        for (var i = 0; i < number.length; i++) {
                          List_number = number[i];
                          if (Driver_currentText_Chang_Pin == List_number) {
                            textEditingController.clear();
                            Alert_Create_Security_PinCode(context);
                          } else {
                            Navigator.of(context).pushAndRemoveUntil(
                                MaterialPageRoute(
                                    builder: (context) =>
                                        Driver_Forgot_Confirm_Pin_Code()),
                                (Route<dynamic> route) => false);
                          }
                        }
                        if (Driver_currentText_Chang_Pin != number) {
                          Navigator.of(context).pushAndRemoveUntil(
                              MaterialPageRoute(
                                  builder: (context) =>
                                      Driver_Forgot_Confirm_Pin_Code()),
                          ...
                          textEditingController.clear();
                          Alert_Security_PinCode(context);
                        }
                      },
                      onChanged: (value) {
                        debugPrint(value);
                ...
                actions: [
                  Column(
                    children: [
                      ...
                      const Text(
                        'รหัส PIN ใหม่ ต้องไม่ตรงกับ รหัส PIN เก่า',
                        style: TextStyle(
...
Future<void> Alert_Create_Security_PinCode(BuildContext context) async {
  showDialog(
      context: context,
      builder: (context) => MediaQuery(
            data: MediaQuery.of(context).copyWith(textScaleFactor: 1),
                  ...
                  children: [
                    ...
                    ElevatedButton(
                      onPressed: () {
                        Navigator.of(context).pushAndRemoveUntil(
                            MaterialPageRoute(
                                builder: (context) => Driver_Forgot_Pin_Code()),
                            (Route<dynamic> route) => false);
                      },
                      child: const Center(
                          child: Text(
